...
void play() {
    ...
    int dy = 0, dx = 0, dr = 0;
    if ((++LoopCut %= 30) == 0)dy = 1;
    if (isTrigger(KEY_D) || isTrigger(KEY_RIGHT))dx = 1;
    if (isTrigger(KEY_A) || isTrigger(KEY_LEFT))dx = -1;
    if (isTrigger(KEY_W) || isTrigger(KEY_UP))dr = 1;
    if (isPress(KEY_S) || isTrigger(KEY_DOWN))FallFlag = 1;
    if (FallFlag)dy = 1;
    Y += dy;
    X += dx;
    R += dr;
...
void over() {
    ...
    textSize(Size);
    text("詰み", Size, Size);
    text("SPACEでリトライ", width / 2 - Size * 3, Size * 20);
